...
#### Step one, review all integer and numeric variables statisitics (note: 'inline histogram' not displaying correctly on output, due to Windows UTF8 limitations):
```{r}
library(skimr)
library(knitr)
library(tidyverse)
...
#TenVars2
```
#### Using createDataPartition() to split data into training and test sets (75% and 25%). Since the outcome variable (CAD) is categorical, this function will make sure that the distribution of outcome variable classes will be similar in both of the sets.
```{r}
library(caret)
...
#### Saved off resulting train and test files for retrieval for subsequent algorithms, etc. This is to ensure that the same training/test split files are used before the recursive feature elimination, cross validation, training and running of algorithms. 
```{r}
...
#### Using 'Recursive Feature Elimination' to find the best subset of atttributes to use for modeling. 
```{r}
library(plyr)
...
fitCntrl <- trainControl(method = "repeatedcv", number = 10, repeats = 10, sampling = "smote")
rfFit <- train(AllvFtrain[,predictors], AllvFtrain[,outcomeName], method = 'rf', returnResamp = "final")
rfFit
plot(rfFit)
...
NBfitCntrl <- trainControl(method = "repeatedcv", number = 10, repeats = 10, sampling = "smote")
NBfit <- train(AllvFtrain[,predictors], AllvFtrain[,outcomeName], method = 'nb', trControl= NBfitCntrl, returnResamp = "final")
NBfit
...
LRfitCntrl <- trainControl(method = "repeatedcv", number = 10, repeats = 10, sampling = "smote")
LRFit <- train(AllvFtrain[,predictors], AllvFtrain[,outcomeName], method = 'glm', family = "binomial", trControl= LRfitCntrl)
LRFit
...
# Ensemble evaluation - work in progress
```{r results='hide', message=FALSE, warning=FALSE}
library(caretEnsemble)

# Defining the training control:

ENCntrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3, search = "grid", savePredictions = "final", sampling = "smote", classProbs = TRUE)

# Training the 3 models:

RFmodel <- train(AllvFtrain[,predictors],AllvFtrain[,outcomeName], method='rf',trControl = ENCntrl,tuneLength = 3)

NBmodel <- train(AllvFtrain[,predictors],AllvFtrain[,outcomeName], method='nb',trControl = ENCntrl,tuneLength = 3)

LRmodel <- train(AllvFtrain[,predictors],AllvFtrain[,outcomeName], method='glm',trControl = ENCntrl,tuneLength = 3)
...
ENCntrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3, search = "grid", savePredictions = "final", sampling = "smote")

ENFit <- caretList(AllvFtrain[,predictors], AllvFtrain[,outcomeName], trControl= ENCntrl, methodList = c("rf", "nb", "glm"))
ENFit
...
#### ACP: For a one unit increase in ACP, the log odds of being diagnosed with Coronary Artery Disease increases by 0.4693 (*Note: Not statistically significant relationship, per the p-value)
#### RWMA: For a one unit increase in RWMA, the log odds of being diagnosed with Coronary Artery Disease increases by 1.8659
#### EF: For a one unit increase in EF, the log odds of being diagnosed with Coronary Artery Disease DECREASES by 1.0746
...
#### Age: For a one unit increase in Age, the log odds of being diagnosed with Coronary Artery Disease increases by 1.1406
...
```{r}
RFstability <- data.frame(Accuracy=rep(0.9467,7), Sensitivity=rep(0.9630, 7), Specificity=rep(0.9048,7))
rownames(RFstability) <- c(3,5,7,10,15,20,25)
RFstability

cols <- c("darkseagreen","cornflowerblue","gray48")
pch <- c(17,18,15)
xmax <- nrow(RFstability) + 2.5
par(mar=c(4,4,1,1))
plot(1:nrow(RFstability), 1:nrow(RFstability), pch="", 
		xlab="nFolds", ylab=NA, xaxt="n", yaxt="n", 
		ylim=c(.9,.975), bty="n", xlim=c(1,xmax), main="Random Forest Stability Results")
for (i in seq(.9,.975,by=.025)) {
...
axis(side=2, at=seq(.9,.975,by=.025), tick=TRUE, 
		las=TRUE, labels=paste(seq(.9,.975,by=.025)))
...
```{r}
NBstability <- data.frame(Accuracy=c(0.9200, 0.9200, 0.9333, 0.9067, 0.9067, 0.9200, 0.9200), Sensitivity=c(0.8889, 0.8889, 0.9074, 0.8704, 0.8889, 0.8889, 0.8889), Specificity=c(1.0, 1.0, 1.0, 1.0, 0.9524, 1.0, 1.0))
rownames(NBstability) <- c(3,5,7,10,15,20,25)
NBstability
...
plot(1:nrow(NBstability), 1:nrow(NBstability), pch="", 
		xlab="nFolds", ylab=NA, xaxt="n", yaxt="n", 
		ylim=c(.85,1.0), bty="n", xlim=c(1,xmax), main="Naive Bayes Stability Results")
for (i in seq(.85,1.0,by=.05)) {
...
axis(side=2, at=seq(.85,1.0,by=.05), tick=TRUE, 
		las=TRUE, labels=paste(seq(.85,1.0,by=.05)))
...
```{r}
LRstability <- data.frame(Accuracy=rep(0.9333,7), Sensitivity=rep(0.9630,7), Specificity=rep(0.8571,7))
rownames(LRstability) <- c(3,5,7,10,15,20,25)
RFstability
...
plot(1:nrow(LRstability), 1:nrow(LRstability), pch="", 
		xlab="nFolds", ylab=NA, xaxt="n", yaxt="n", 
		ylim=c(.85,.975), bty="n", xlim=c(1,xmax), main="Logistic Regression Stability Results")
for (i in seq(.85,.975,by=.025)) {
...
axis(side=2, at=seq(.85,.975,by=.025), tick=TRUE, 
		las=TRUE, labels=paste(seq(.85,.975,by=.025)))
...
```{r}
Duration <- data.frame(Rforest=c(6.97, 6.95, 6.97, 6.85, 6.91, 6.89, 7.14), NBayes=c(5.11, 6.39, 7.39, 8.86, 12.14, 14.50, 18.30), LRegression=c(2.84, 3.77, 4.99, 5.50, 7.77, 9.61, 11.25))
rownames(Duration) <- c(3,5,7,10,15,20,25)
Duration

cols <- c("green3","indianred","gold")
pch <- c(17,18,15)
xmax <- nrow(Duration) + 2.5
par(mar=c(4,4,1,1))
plot(1:nrow(Duration), 1:nrow(Duration), pch="", 
		xlab="nFolds", ylab=NA, xaxt="n", yaxt="n", 
		ylim=c(2,20), bty="n", xlim=c(1,xmax), main="Algorithm Duration (Seconds)")
...
axis(side=2, at=seq(2,20,by=2), tick=TRUE, 
		las=TRUE, labels=paste(seq(2,20,by=2)))
