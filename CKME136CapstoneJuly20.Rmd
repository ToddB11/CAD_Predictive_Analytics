...
```
#### Included the following chunk to create a repeatable 75/25 split for evaluation of training/testing results (that is, for running and capturing multiple runs, with different folds, etc ):
```{r}

#AllvFtrainCopy2 <- AllvFtrain
#AllvFtestCopy2 <- AllvFtest

#write.csv(AllvFtrainCopy2,'C:\\Users\\TGB\\Documents\\Ryerson\\CKME 136\\RMD Versions\\AllvFtrainCopy2.csv')

#write.csv(AllvFtestCopy2,'C:\\Users\\TGB\\Documents\\Ryerson\\CKME 136\\RMD Versions\\AllvFtestCopy2.csv')
```
...
```
returnResamp = "final", tuneLength = 5
...
rfFit <- train(AllvFtrain[,predictors], AllvFtrain[,outcomeName], method = 'rf', returnResamp = "final", tuneLength = 5, trControl = trainControl(method = "repeatedcv", number = 10, repeats = 10, sampling = "smote"))
rfFit
plot(rfFit)
...
NBfit <- train(AllvFtrain[,predictors], AllvFtrain[,outcomeName], method = 'nb', returnResamp = "final", tuneLength = 5, trControl = trainControl(method = "repeatedcv", number = 10, repeats = 10, sampling = "smote"))
NBfit
...
LRFit <- train(AllvFtrain[,predictors], AllvFtrain[,outcomeName], method = 'glm', family = "binomial", trControl = trainControl(method = "repeatedcv", number = 10, repeats = 10, sampling = "smote"))
LRFit
...
ENCntrl <- trainControl(method = "repeatedcv", number = 10, repeats = 10, search = "grid", savePredictions = "final", sampling = "down")

# Training the 3 models:

RFmodel <- train(AllvFtrain[,predictors],AllvFtrain[,outcomeName], method='rf',tuneLength = 5, trControl = ENCntrl)

NBmodel <- train(AllvFtrain[,predictors],AllvFtrain[,outcomeName], method='nb',tuneLength = 5, trControl = ENCntrl)

LRmodel <- train(AllvFtrain[,predictors],AllvFtrain[,outcomeName], method='glm',tuneLength = 5, trControl = ENCntrl)
