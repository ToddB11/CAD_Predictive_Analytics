...
# Taking the top 5 predictors:
```{r}
predictors <-c("TCP", "ACP", "RWMA", "EF", "NCP", "Age", "HTN", "DM")
```
...
```{r warning=FALSE}
logiTrain <- data.frame(AllvFtrain[,c(23,26,51,50,27,1,5,4,53)])

logiTrain$RWMA <- factor(logiTrain$RWMA, levels = c("Med", "High"), ordered = TRUE)
logiTrain$Age <- factor(logiTrain$Age, levels = c("Med", "High"), ordered = TRUE)
logiTrain$EF <- factor(logiTrain$EF, levels = c("Low", "Med"), ordered = TRUE)
